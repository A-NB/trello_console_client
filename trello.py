"""
Интерактивное консольное приложение для управления доской на сайте trello.com при помощи API Trello.
...
булева признака (True или False) для дальнейшего использования его в вызывающей эту функцию программе.
"""            
...
def create_column(par_column_name):
    ...
    if column_id or stop: # Если на доске найдены колонки с похожим именем:
...
def find_card(name, action):
    ...
    if len(card_ids) > 1: # Если найдено более одной карточки с именем name, ...
        ...
        for i in range(len(card_ids)):
            # ... и перечисляем их в виде нумерованного списка:
            print(f"{str(i + 1)}: '{card_ids[i][1]}' в колонке {card_ids[i][3]} из {len(column_data)} '{card_ids[i][2]}' (id: {card_ids[i][0]}, метки: {card_ids[i][4]}, цвет фона: {card_ids[i][5]}, фоновое изображение: {'есть' * bool(card_ids[i][6]) + 'нет' * (not card_ids[i][6])}, последняя активность: {card_ids[i][7]})")            
